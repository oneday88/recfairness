...
class CFTrainer(object):
    ...
    def topK_evaluator(self, testDt, recommendedDt , KList=[20,50,100,200],user_batch_size=64, evalCount=None):
        testUserId = testDt['userId'].unique()
        if evalCount is not None:
            testUserId = np.random.choice(test_userId,eval_count)
        testMlogId = testDt[['mlogId','userClickCount','userZanCount']].drop_duplicates().values
        
        predDt = self.predict(testUserId, testMlogId, recommendedDt, user_batch_size)
        predDt = pd.merge(predDt, recommendedDt, how='left', on=['mlogId','userId'])
        predDt = predDt[~(predDt['isClick']>=0)]
        predDt.drop(['isClick'], axis=1, inplace=True)

        for K in KList:
            hitK, precK, recallK, nDCGK = metrics_at_k(testDt, predDt, K)
    ...
    def fit(self, train_X, train_Y, params_dict, test_data=None, recommendedDt=None, rankingScoreDt=None):
        ### The parameters
