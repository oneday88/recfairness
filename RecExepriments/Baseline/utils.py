...
"""
Get the top100 popularity items
"""
def topK_popular_items(df, K=100):
    df['popularity'] = 0.5*df['userImprssionCount']+0.5*df['userClickCount']
    popularity_items = df.groupby(['creatorId','mlogId'])['popularity'].agg(['sum']).reset_index()
    popularity_items.columns = ['creatorId','mlogId','popularity']
    popularity_items = popularity_items.nlargest(K,'popularity')
