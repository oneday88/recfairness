...
logging.basicConfig(level=logging.DEBUG, filename="baselineRandomWithChurn.log", filemode="w+",format="%(asctime)-15s %(levelname)-8s %(message)s")
...
itemCandidates = impressions.loc[impressions.publishDt<16,'mlogId'].unique()
...
metricsDt.to_csv("dataChurnRandom.csv", index=False, sep=',',float_format='%.5f')
